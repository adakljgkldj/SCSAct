...
def print_all_results(results, datasets, method):
    ...
    a = []
    for result, dataset in zip(results,datasets):
        a.append(round(result['FPR']*100,2))
        a.append(round(result['AUROC']*100, 2))
        a.append(round(result['AUIN']*100, 2))
...
def compute_traditional_ood(base_dir, in_dataset, out_datasets, model_arch,method, name, m,n,args=None):
    known = np.loadtxt('{base_dir}/{in_dataset}/{model}/{method}/{name}/in_scores.txt'.format(base_dir=base_dir, in_dataset=in_dataset, model=model_arch, method=method, name=name), delimiter=',')
    ...
    if method == 'rowl':
        ...
        print("threshold 0.05:",threshold)
    ...
    for out_dataset in out_datasets:
        novel = np.loadtxt('{base_dir}/{in_dataset}/{model}/{method}/{name}/{out_dataset}/out_scores.txt'.format(base_dir=base_dir, in_dataset=in_dataset, model=model_arch,method=method, name=name, out_dataset=out_dataset), delimiter=',')
...
def compute_in(base_dir, in_dataset, model_arch,method, name):

    known_nat = np.loadtxt('{base_dir}/{in_dataset}/{model}/{method}/{name}/in_scores.txt'.format(base_dir=base_dir, in_dataset=in_dataset, model = model_arch,method=method, name=name), delimiter=',')
    ...
    known_nat_label = np.loadtxt('{base_dir}/{in_dataset}/{model}/{method}/{name}/in_labels.txt'.format(base_dir=base_dir, in_dataset=in_dataset, model= model_arch,method=method, name=name))
    ...
    print('FNR: {fnr:6.2f}, Acc: {acc:6.2f}, End-to-end Acc: {eteacc:6.2f}'.format(fnr=known_nat_cond_fnr*100,acc=known_nat_acc*100,eteacc=known_nat_eteacc*100))
